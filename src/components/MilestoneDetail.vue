<template>
    <div>
        ...
        <div class="buttons" >
            ...
            <el-button type="primary" @click="onEditMilestonClick" >
        ...
        <div class="subtitle">
            ...
            <el-collapse v-model="activeNo" >
                <el-collapse-item  name="0">
                    <template #title>
                        <el-tag type="danger">P0</el-tag>&nbsp;&nbsp;(已完成{{customInfo.p0issue[0]}}/总计{{customInfo.p0issue[0]+customInfo.p0issue[1]}})
                    </template>
                    ...
                    <div v-for="issue in p0" :key="issue.id">
                        <div class="issueitem" v-if="issue.state=='closed'">
                            ...
                            子需求:{{issue.title}}
                            <el-button type="warning" style="float:right" @click="gotoGitlab(issue.web_url)">前往gitlab查看</el-button><br>
                            计划结束时间 {{issue.due_date}} &nbsp;&nbsp;&nbsp;&nbsp;结束时间：{{getTime(issue.closed_at)}}
                        ...
                        <div class="issueitem" v-if="issue.state=='opened'">
                            ...
                            子需求:{{issue.title}}
                            <div style="displat:inline-block;float:right">
                                <!--el-button type="danger" >强行关闭</el-button-->
                                <el-button type="warning" style="float:right" @click="gotoGitlab(issue.web_url)">前往gitlab查看</el-button>
                            </div><br>
                            计划结束时间 {{issue.due_date}}  &nbsp;&nbsp;&nbsp;&nbsp;    
                        </div>
                    </div>
                </el-collapse-item>
                <el-collapse-item  name="1">
                    <template #title>
                        <el-tag type="warning">P1</el-tag>&nbsp;&nbsp;(已完成{{customInfo.p1issue[0]}}/总计{{customInfo.p1issue[0]+customInfo.p1issue[1]}})
                    </template>
                ...
                <el-collapse-item  name="2">
                    <template #title>
                        <el-tag>P2</el-tag>&nbsp;&nbsp;(已完成{{customInfo.p2issue[0]}}/总计{{customInfo.p2issue[0]+customInfo.p2issue[1]}})
                    </template>
        ...
        <div class="subtitle">
            <el-collapse >
                ...
                <template #title>
                    ...
                    <div style="display:inline;font-size:20px">Merge Request 总数{{customInfo.mrinfo[0]+customInfo.mrinfo[1]+customInfo.mrinfo[2]}}</div>
                ...
                <div class="issueitem" v-for="mr in info.mrs" v-bind:key="mr.id">
                    MR:{{mr.title}}
                    <el-button type="warning" style="float:right" @click="gotoGitlab(mr.web_url)">前往gitlab查看</el-button><br>
...
export default {
    ...
    methods:{
        onEditMilestonClick(){
            this.$router.push('/project/' + this.projectId+'/editmilestone/'+this.milestoneId)
        },
        onCreateDemandClick(){
            this.$router.push('/project/' + this.projectId+'/milestone/'+this.milestoneId+"/createdemand")
        },
        getTime(str){
    ...
    computed:{
        ...
        p0:function(){
            ...
            for(let i=0;i<this.info.issues.length;i++){
                if(this.info.issues[i].labels.indexOf("P0")>-1 &&
        ...
        p1:function(){
            let res=[]
            for(let i=0;i<this.info.issues.length;i++){
                if(this.info.issues[i].labels.indexOf("P1")>-1 &&
                this.info.issues[i].labels.indexOf("feature")>-1){
                    res.push(this.info.issues[i])
                }
            }
        ...
        p2:function(){
            let res=[]
            for(let i=0;i<this.info.issues.length;i++){
                if(this.info.issues[i].labels.indexOf("P2")>-1 &&
                this.info.issues[i].labels.indexOf("feature")>-1){
                    res.push(this.info.issues[i])
                }
            }
